...
> **Khuyến nghị mạnh:** trước buổi demo, chạy **1 lệnh duy nhất** sau để hệ thống "no-warmup":
>
> ```powershell
> python finwatch\scripts\prepare_demo_full.py
> ```
>
> Trong ~27 giây script sẽ: register Debezium connector, verify snapshot, **seed 30 ngày baseline cho z-score**, drive 1500 txn live load, **bắn cả 6 kịch bản fraud** (R1-R6 đều có count > 0 ngay khi mở `/fraud`), pre-warm 6 trang UI + 16 API, và in báo cáo sẵn sàng. Xem chi tiết ở `scripts/prepare_demo_full.py`. Sau bước này, mọi widget trong UI đã có data đẹp trước khi khán giả vào.

---

## 0. Mở 8 tab trình duyệt (1 lần duy nhất)

Mở 8 tab theo thứ tự dưới đây — sẽ chuyển qua lại trong lúc demo:

| Tab | URL | Đặt tên tab |
|---|---|---|
| 1 | http://localhost:3002 | **Dashboard** |
| 2 | http://localhost:3002/architecture | **Architecture** |
| 3 | http://localhost:3002/demo | **Insert & trace** |
| 4 | http://localhost:3002/trace | **Trace** |
| 5 | http://localhost:3002/fraud | **Fraud rules** |
| 6 | http://localhost:3002/accounts | **Accounts** *(mới)* |
| 7 | http://localhost:3002/alerts | **Alerts** *(mới)* |
| 8 | http://localhost:3002/kafka | **Kafka** |

Trong NavBar trên cùng có sẵn 8 link tương ứng — có thể click trực tiếp thay vì gõ URL.
...
├─────────────────────────────────────────────────────────────┤
│  Architecture Diagram                  │   Health KPIs      │
...
- Trong **Architecture Diagram** ở trên cùng, các "hạt" nhấp nháy chạy từ PG → CH.
...
## 2. Tab "Architecture" (http://localhost:3002/architecture) — flow thị giác

Trang này hiển thị **4 node** (PostgreSQL, Debezium, Kafka, ClickHouse) nối với nhau bằng các đường, và mỗi giao dịch là một **hạt sáng** chạy dọc đường nối.

### 2.1 Quy ước hiển thị (in trên trang)
...
1. Quay lại tab **Dashboard**, click `wire-fraud` (250M VND).
2. Chuyển sang tab **Architecture** ngay lập tức.
3. Trong vòng ~1 giây sẽ thấy một hạt **to + quầng đỏ pulsing** đi từ PG sang CH.
4. Đây là cách giải thích trực quan: "CDC bắt giao dịch lớn ở PG và bằng chứng đi qua từng chặng".

Có thể click `Drive normal load (200 txns)` bên Dashboard để thấy hàng chục hạt nhỏ chạy song song.

---

## 3. Tab "Insert & trace" (http://localhost:3002/demo) — tự tay drive pipeline
...
├─────────────────────────┬────────────────────────────────────┤
│  Insert transaction     │   Architecture flow                │
...
6. Sau ~1 giây giao dịch hiện trên CH. Mở tab **Architecture** — sẽ thấy hạt to có quầng đỏ pulse.
7. Mở tab **Dashboard** — Alert feed có thêm dòng `LARGE_AMT high … Single txn 120M VND`.
...
| Phút | Tab | Hành động |
|---|---|---|
| 0–1 | Dashboard | Giới thiệu 4 widget. Click `Drive normal load (200 txns)`. Quan sát TPS spike + Live stream chạy. |
| 1–2 | Architecture | Quan sát hạt sáng chạy giữa các node. Số EPM ở mỗi node nhảy. |
...
Đợi ~27s. Khi banner cuối in `READY for COMPREHENSIVE DEMO` + `HTTP probes 22/22` + `Fraud rules firing 6/6`, hệ thống đã sẵn sàng. Mở http://localhost:3002 là demo được ngay — không cần làm gì thêm.
...
| `/fraud` các card đều show count = 0 | Chưa inject fraud. Quay lại Dashboard, click ít nhất 1 fraud button. |
...
| Architecture EPM counters | `GET /api/pipeline-stats` | 1s |
...
- Bằng chứng quan sát được ở từng chặng (Architecture particles, Per-hop trace, Kafka browser).
